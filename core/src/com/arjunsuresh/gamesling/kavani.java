...
allset=-1;
 ...
 { Vector3f vec=target.position();
  ...
  public  void setStone(String kavanistr,String  stonestr,String target)
    {
...
//((Text)scene.objects.get("debu")).text(" "+stone.position().y);
    if(stone.position().y>28f||stone.position().x>15f||stone.position().z>17f||stone.position().x<-15f||stone.position().z<-4f||stone.position().y<-5f||stone.velocity()==new Vector3f(0,0,0))
    {
    if(stone_rem==0)
    {
     pthis.gameover();
